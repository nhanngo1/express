$(function() {
    ...
    function updateUserTable(data) {
        ...
        $.each(data, (index, user) => {
            if (user.login != 'undefined') {
                ...
                response += '               <td class="action"><button class="fa fa-edit btn btn_edit" id="edt_' + index + '_' + user.login + '">';
                response += '               <button type="button" class="fa fa-trash btn btn_delete" id="del_' + index + '_' + user.login + '">';
                response += '               <button type="button" class="fa fa-save btn btn_save" id="sav_' + index + '_' + user.login + '">';
                response += '               <button type="button" class="fa fa-ban btn btn_cancel" id="can_' + index + '_' + user.login + '"></td>';
                response += '           </tr>';
            }
        });
    ...
    function getIndexAndLoginAccount(id) {
    ...
    $("#userdata").on('click', '.btn_delete', function(e) {
        currentRows = $('#userdata tr').length;
        let infoArr = getIndexAndLoginAccount(e.target.getAttribute('id'))
        console.log("click delete")
        clearStatus();
        deleteUser(infoArr[1]);
    })
